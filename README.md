# Tyler Windes

**Technical Consultant | Systems Analyst | Business Systems Analyst**

I turn ambiguous operational problems into clear requirements, practical workflows, implementation-ready documentation, and results people can trace and review. My work connects systems analysis, data-quality rules, SQL, testing, and straightforward communication.

**Portfolio:** [tyler-windes.github.io](https://tyler-windes.github.io/)

## Featured work

### [Workflow Intake Analysis Demo](https://github.com/Tyler-Windes/workflow-intake-analysis-demo)

A Python and SQL project for turning inconsistent synthetic intake into validated, traceable results. I designed it around a simple principle: uncertainty should remain visible rather than being converted into an unsupported fact.

- translated requirements and acceptance criteria into executable rules;
- separated valid and review-required records;
- preserved raw-to-normalized traceability;
- reconciled SQLite, SQL exports, and generated reports; and
- used JSON Schema, OpenAPI, Postman, automated tests, and CI.

[View the v1.0.0 release](https://github.com/Tyler-Windes/workflow-intake-analysis-demo/releases/tag/v1.0.0)

## How I work

I value clear reasoning, honest limitations, and evidence that can be checked. I use AI-assisted tools for brainstorming, drafting, refactoring, test ideas, and consistency review, while keeping human judgment and final accountability explicit.

## Background

My background combines regulated business operations, client-facing problem solving, process design, documentation, and technical systems training. That perspective keeps my technical work focused on practical decisions and usable handoffs.
